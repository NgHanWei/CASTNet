from cross_validation_cog import CrossValidation
import numpy as np
...
args = {
        "mode": 'train',
        "data_dir": "F:/HR DATA 50 SUBJECTS/CORRECTED_openclose/",
        "acc": None,
        "std": None,
        "results": None,
...
def subject_independent_data(subj):
    ...
    for i in range (0,50):
        if i == subj:
            ...
            target_data = np.moveaxis(target_data,2,0)
            ...
            target_data = target_data[:len(target_data)-20]
            target_onehot_labels = target_onehot_labels[:len(target_onehot_labels)-20]
            print(target_data.shape)
            print(target_onehot_labels.shape)
        else:
            data,labels = CV.load_data_per_subject_mat_fbcnet(sub=i)
...
def get_data_openclose(sub):
    ...
    data_valid = data[len(data)-20:]
    onehot_labels_valid = onehot_labels[len(onehot_labels)-20:]

    data_train = data[:len(data)-20]
    onehot_labels_train = onehot_labels[:len(onehot_labels)-20]
...
def train(dataset_conf, train_conf, results_path):
    ...
    for sub in range(29,n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        for train in range(n_train): # How many repetitions of training for subject i.
            ...
            history = model.fit(X_train, y_train_onehot, validation_data=(X_train, y_train_onehot), 
            ...
            # history = model_2.fit(np.concatenate((new_data,X_train),axis=0), np.concatenate((new_labels,y_train_onehot),axis=0), validation_data=(X_test, y_test), 
...
def run():
    ...
    train_conf = { 'batch_size': 16, 'epochs': 100, 'patience': 300, 'lr': 0.00001,
                  'LearnCurves': False, 'n_train': 3, 'model':'ATCNet'}
